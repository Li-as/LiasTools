...
public class BuildsWindow : EditorWindow
{
    ...
    private IEnumerator PerformBuild(List<CustomBuildSettings> buildSettings)
    {
        ...
        string originalSpatializer = AudioSettings.GetSpatializerPluginName();
        ...
        for (int i = 0; i < buildSettings.Count; i++)
        {
            ...
            if (BuildIndividualTarget(curBuildSettings) == false)
            {
                ...
                TryRestoreOriginalSettings(originalTarget, originalTargetGroup, originalSubtarget, originalProductName, originalSpatializer);

                yield break;
            }

            if (i == buildSettings.Count - 1)
                EditorUtility.OpenWithDefaultApp(curBuildSettings.BuildPath);
    ...
    private bool BuildIndividualTarget(CustomBuildSettings buildSettings)
    {
        ...
        if (buildSettings.DisableSpatializer)
        ...
        Debug.Log($"Making build with options: target is {options.target}; " +
                  ...
                  $"build options is {options.options}");
        ...
        if (targetGroup == BuildTargetGroup.Standalone)
        {
            if (options.subtarget == (int)StandaloneBuildSubtarget.Player)
            {
                ...
                if (options.target != curBuildTarget || options.subtarget != originalSubtarget)
    ...
    private void TryRestoreOriginalSettings(BuildTarget origTarget, BuildTargetGroup origTargetGroup, int origSubtarget, string origProductName, string origSpatializer)
    {
        PlayerSettings.productName = origProductName;
        AudioSettings.SetSpatializerPluginName(origSpatializer);
